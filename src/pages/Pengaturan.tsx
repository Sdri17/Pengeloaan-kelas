...
import { Save, Plus, Trash2, UserPlus, ShieldAlert, Key, Pencil, X, Download, Upload, Database } from 'lucide-react';
...
export default function Pengaturan({ settings, setSettings, role, currentUser }: { settings: SettingsType | null, setSettings: (s: SettingsType | null) => void, role: 'guru' | 'kepsek', currentUser?: AppUser }) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => ({ ...prev, [name]: name.startsWith('bobot') ? Number(value) : value }));
  };
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto custom-scrollbar pr-2">
                {(formData.mata_pelajaran || []).map((mapel, index) => (
                  <div key={index} className="flex gap-2">
                    ...
                    />
                    {role === 'guru' && (
                      <button type="button" onClick={() => removeMapel(index)} className="p-2 text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 rounded-xl transition-colors">
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
