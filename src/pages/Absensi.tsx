...
import { Download, Save, Calendar, Trash2, Plus, Info, AlertTriangle, CheckSquare } from 'lucide-react';
...
export default function Absensi({ semester, role, settings, setSettings }: AbsensiProps) {
  ...
  // Helper to determine if a date is Sunday or a custom holiday
        ...
        if (status) {
          ...
          if (existing) {
            if (existing.status !== status) {
              ...
              await store.attendance.setItem(existing.id, existing);
            }
          } else {
            ...
            await store.attendance.setItem(newAtt.id, newAtt);
          }
        }
      }
  ...
  const exportPDF = async () => {
    ...
    doc.text(`Rekap Absensi - Semester ${semester}`, 14, 15);
    doc.setFontSize(10);
    doc.text(`Periode: ${rekapFilter} | Kelas: ${filterClass || 'Semua Kelas'}`, 14, 22);
    ...
    const startXRight = isDetailed ? 200 : 140;
    ...
    doc.text(`${city}, ${today}`, startXRight, sigY);
    doc.text('Guru Kelas', startXRight, sigY + 5);
    doc.text(settings?.nama_wali_kelas || '(.........................)', startXRight, sigY + 30);
    doc.text(`NIP. ${settings?.nip_wali_kelas || '.........................'}`, startXRight, sigY + 35);
        ...
        {activeTab === 'Rekap' && (
          <table className="w-full text-sm text-left border-collapse">
            <thead className="text-xs uppercase bg-slate-800/90 sticky top-0 backdrop-blur-sm z-10 text-slate-400">
              <tr>
                <th className="px-4 py-4 border-b border-slate-700/50 w-12 font-medium">No</th>
                <th className="px-4 py-4 border-b border-slate-700/50 w-16 font-medium">Kelas</th>
                <th className="px-4 py-4 border-b border-r border-slate-700/50 sticky left-0 bg-slate-800/95 backdrop-blur-md shadow-[1px_0_0_0_rgba(51,65,85,0.5)] font-bold z-20 min-w-[160px]">Nama Siswa</th>
                ...
                {isDetailed && datesList.map(d => {
                  ...
                  return (
                    <th 
                      key={dStr} 
                      className={`px-1 py-3 border-b border-r border-slate-700/50 text-center min-w-[38px] text-[10px] ${
                ...
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-indigo-400 font-semibold text-xs">Aktif</th>
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-emerald-400 font-semibold text-xs">Hadir</th>
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-amber-400 font-semibold text-xs">Sakit</th>
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-sky-400 font-semibold text-xs">Izin</th>
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-rose-400 font-semibold text-xs">Alpa</th>
                <th className="px-3 py-4 border-b border-slate-700/50 text-center w-16 text-slate-400 font-semibold text-xs">Libur</th>
                <th className="px-4 py-4 border-b border-slate-700/50 text-center w-24 text-indigo-300 font-bold text-xs">% Hadir</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700/50">
              {rekapData.length === 0 ? (
                ...
                rekapData.map((student, index) => (
                  <tr key={student.id} className="hover:bg-slate-700/20 transition-colors group">
                    ...
                    <td className="px-4 py-3.5 text-slate-400">{student.kelas}</td>
                    <td className="px-4 py-3.5 border-r border-slate-700/50 font-medium text-slate-200 sticky left-0 bg-slate-800/40 backdrop-blur-md shadow-[1px_0_0_0_rgba(51,65,85,0.5)] z-10 group-hover:bg-slate-700/50 transition-colors">
                      {student.nama}
                    </td>
                    ...
                    {isDetailed && datesList.map(d => {
                      ...
                      if (hol.isLibur) {
                        return (
                          <td 
                            key={dStr} 
                            className="px-1 py-3.5 border-r border-slate-700/50 text-center text-xs font-bold text-rose-400/80 bg-rose-500/5 select-none"
                            title={hol.desc}
                          >
                            L
                          </td>
                      ...
                      return (
                        <td key={dStr} className={`px-1 py-3.5 border-r border-slate-700/50 text-center text-xs ${statusStyle}`}>
                          {statusLetter}
                        </td>
                      );
                    })}
                    ...
                    {/* Kolom rincian akhir */}
                    <td className="px-3 py-3.5 text-center text-indigo-300 font-mono font-medium">{student.HariAktif}</td>
                    <td className="px-3 py-3.5 text-center text-emerald-400 font-mono font-bold bg-emerald-500/5">{student.Hadir}</td>
                    <td className="px-3 py-3.5 text-center text-amber-400 font-mono bg-amber-500/5">{student.Sakit}</td>
                    <td className="px-3 py-3.5 text-center text-sky-400 font-mono bg-sky-500/5">{student.Izin}</td>
                    <td className="px-3 py-3.5 text-center text-rose-400 font-mono bg-rose-500/5">{student.Alpa}</td>
                    <td className="px-3 py-3.5 text-center text-slate-400 font-mono font-medium">{student.Libur}</td>
                    <td className="px-4 py-3.5 text-center text-indigo-300 font-mono font-bold bg-indigo-500/5">
                ...
                {role === 'guru' ? (
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full mt-2 py-3 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold text-sm rounded-xl transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 cursor-pointer"
                    >
